...
class GoogleAuthenticatableTest < ActiveSupport::TestCase
  ...
  test 'testing token validation' do
    user = new_user
    assert_not user.validate_token('1')
    assert_not user.validate_token(ROTP::TOTP.new(user.get_qr).at(Time.now))
  ...
  test 'requiring token after remembertime' do
    ...
    assert_not u.require_token?(u.email + "," + 1.day.ago.to_i.to_s)
